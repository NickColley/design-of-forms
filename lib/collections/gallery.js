...
module.exports = function (markdown) {
    return function gallery (collectionApi) {
        return sortedPages(collectionApi).flatMap(page => {
            const markdownContent = page.template.inputContent.replace(/{{ page\.url }}/g, '');
            ...
            return markdown
                        ...
                        // Unique images only
                        .filter((token, index, originalTokens) => {
                            return originalTokens
                                    .map(originalToken => originalToken.src)
                                    .indexOf(token.src) === index;
                        })
        })
    }
}
